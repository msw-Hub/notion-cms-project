...
export default defineConfig(({ mode }) => {
  ...
  // 개발 전용 Notion API 프록시. 브라우저는 이 자체 엔드포인트(/notion-proxy/**)만 알면 되고,
  // 실제 Notion 토큰/버전 헤더는 여기(Node 서버 측)에서만 주입한다. Notion API는 CORS를 지원하지
  ...
  return {
    ...
    // `server`(vite dev)와 `preview`(vite preview, 빌드 산출물 로컬 확인용) 양쪽 모두에
    // 동일한 프록시를 건다 — `preview`에만 없으면 빌드 검증 시 Notion 호출이 전부 실패한다.
    server: { proxy: notionProxy },
