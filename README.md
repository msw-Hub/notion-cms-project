...
4. (선택) 용어 목록 화면에 "Notion에서 용어 추가" 바로가기 버튼을 띄우려면 같은 Production
   환경에 `VITE_NOTION_DATABASE_URL`도 등록한다. 이 변수는 앞의 두 변수와 달리 비밀값이 아니므로
   **`VITE_` 접두사를 의도적으로 붙인다** — 실수가 아니니 지우지 않는다. 마찬가지로 등록 후
   재배포해야 반영된다(`VITE_` 변수는 빌드 타임에 번들로 인라인되기 때문).
...
| `VITE_NOTION_DATABASE_URL` | 선택   | 용어 목록 화면의 "Notion에서 용어 추가" 바로가기 버튼이 열 Notion 데이터베이스 페이지 URL. 통합 토큰과 달리 비밀값이 아니라 의도적으로 `VITE_` 접두사를 붙였다. 값을 비워두면 버튼이 렌더링되지 않는다. |
