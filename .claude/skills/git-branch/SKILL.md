...
- 새 브랜치 생성과 동시에 전환
- 브랜치명 규칙 자동 검증
- 프리픽스 자동 제안 (feature/, fix/, hotfix/, docs/, chore/, refactor/, test/ — 아래 "브랜치 네이밍 규칙" 목록과 동일)
...
1. 병합 상태 확인
2. 원격에도 같은 이름의 브랜치가 있는지 확인 (있으면 "로컬만 삭제되며, 원격 삭제는 별도 요청이 필요하다"고 안내 — 이 스킬엔 push 권한이 없어 원격 삭제는 하지 않음)
3. 안전 확인 후 로컬 브랜치 삭제
...
3. **브랜치명 규칙 위반** → 자동 제안 받아 수정

### 복구 방법

- 잘못 삭제한 브랜치: `git reflog`로 확인 가능한 커밋 SHA를 안내하고, 복구 여부는 사용자에게 확인한다
- 손실된 변경사항: `git stash list`에서 복구
- 꼬인 브랜치 상태: 이 스킬에는 `git reset` 권한이 없으므로, 필요한 명령을 안내만 하고 직접 실행하지 않는다
